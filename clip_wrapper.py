...
MODEL_DIM = 512
ZERO_SHOT_WEIGHTS_PATH = "zeroshot_weights.pickle"
...
class ClipWrapper:
    ...
    def generate_zero_shot_weights(self):
        if os.path.exists(ZERO_SHOT_WEIGHTS_PATH):
            with open(ZERO_SHOT_WEIGHTS_PATH, 'rb') as handle:
                zeroshot_weights = pickle.load(handle)
        ...
        for classname in tqdm(IMAGENET_CLASSES):
            ...
            class_embeddings = self.texts2vec(texts)
